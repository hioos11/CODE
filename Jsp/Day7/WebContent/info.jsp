...
<%
/* 	// 세션에 저장된 애들
	// 세션에 있는 값은 String값이 아니다.(세션 객체일 뿐)
	// 따라서 String으로 바꿔주고, idx는 int로 한번 더 바꿔주고 set해준다. */
...
<body>
    ...
    <form name="regform" id="regform" method="post" action="info_ok.jsp" onsubmit="return sendit()">
        ...
        <%				
        	// getHobby로 가져온 배열에 "드라이브"가 있는지에 따라 true, false가 리턴됨
        ...
        <%
			if(compare_hobby("등산", member.getHobby())){
       ...
       <%
        ...
        <input type="text" name="ssn2" id="ssn2" maxlength="7" value="<%=member.getSsn2()%>"> <input type="button" id="ssnBtn" value="주민등록번호 검증"></p>
        <p><label>우편번호 : <input type="text" name="zipcode" id="sample6_postcode" value="<%=member.getZipcode()%>" placeholder="우편번호" readonly></label> 
